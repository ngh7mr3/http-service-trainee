...
async def validate_ip(ip: str):
    try:
        ...
        for byte in byte_list:
            if len(byte) > 3 or len(byte) < 1:
                return False
            elif int(byte) > 255 or int(byte) < 0:
                return False
    except Exception:
        return False
...
async def ip_checkpoint(request, handler):
    ...
    try:
        ...
        if not request_ip:
            ans = f"Bad ip provided in header: {_ip}"
    ...
    if current_timeout > -1:
        print(f"{masked_ip} is still timed out for {current_timeout}")
...
def initialize_app(
    ...
    redis_timeout_db: int,
):

    app = web.Application(middlewares=[ip_checkpoint])
    app.add_routes(
        [
            web.route("*", "/foo", content_handler),
